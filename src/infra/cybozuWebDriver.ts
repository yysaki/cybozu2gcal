...
export class CybozuWebDriver implements IWebDriver {
  async evaluate(): Promise<EvaluateOutputData[]> {
    ...
    try {
      ...
      const page = await this.openSchedulePage(browser);
      return await this.evaluateWebPage(page);
    } finally {
      if (browser !== null) {
        await browser.close();
      }
  ...
  private async openSchedulePage(browser: Browser): Promise<Page> {
  ...
  private async evaluateWebPage(page: Page): Promise<EvaluateOutputData[]> {
